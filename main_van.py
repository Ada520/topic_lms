...
parser.add_argument('--domain', type=str, default='apnews',
                    help='with additional topic embedding')
parser.add_argument('-f', '--en1-units',
                    type=int,   default=100)
parser.add_argument('-s', '--en2-units',
                    type=int,   default=100)
...
def train():
    ...
    for batch_n in range(0, data_len-args.batch_size, args.batch_size):
        ...
        padded = np.array(list(itertools.zip_longest(*sub, fillvalue=0))).T
        van_inp = np.array([to_onehot(doc.astype('float'), len(sorted_wc)) for doc in padded if np.sum(doc) != 0])
        print (van_inp[0])
        #print (padded.shape)
        targets = np.roll(padded, -1)
        targets[:, -1] = 0
        ...
        topic_var = prod_lda.decoder.weight.data
        if args.mit_topic:
